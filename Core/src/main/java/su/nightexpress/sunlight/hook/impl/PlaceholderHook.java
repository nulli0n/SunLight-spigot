        ...
        public String onPlaceholderRequest(Player player, @NotNull String params) {
            try {
                ...
                if (prefix.equalsIgnoreCase("afk")) {
                    ...
                    if (kit != null)
                        return String.valueOf(user.getCooldown(kit).map(CooldownInfo::getExpireDate).orElse(0L));

                    kit = this.getKit(module, rest, "cooldown_");
                    if (kit != null)
                        return user.getCooldown(kit).map(c -> TimeUtil.formatTimeLeft(c.getExpireDate())).orElse("-");
                    ...
                    if (warp != null)
                        return String.valueOf(user.getCooldown(warp).map(CooldownInfo::getExpireDate).orElse(0L));

                    warp = this.getWarp(module, rest, "cooldown_");
                    if (warp != null)
                        return user.getCooldown(warp).map(c -> TimeUtil.formatTimeLeft(c.getExpireDate())).orElse("-");
