...
public class HomesCache {
    ...
    public void initialize() {
        this.module.plugin().getData().getHomes().forEach(this::cache);
        this.getUserHomes().keySet().removeIf(userName -> {
            if (!this.module.plugin().getData().isUserExists(userName)) {
                this.module.plugin().getData().deleteHomes(userName);
                this.module.info("Deleted homes data of invalid user '" + userName + "'.");
                this.getPublicHomes().remove(userName);
                this.getInvitesHomes().remove(userName);
                return true;
            }
            return false;
        });
