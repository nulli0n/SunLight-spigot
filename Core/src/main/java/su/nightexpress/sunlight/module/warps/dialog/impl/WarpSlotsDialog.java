...
public class WarpSlotsDialog extends Dialog<Warp> {
    ...
    private static final TextLocale TITLE = LangEntry.builder("Warps.Dialog.WarpSlots.Title").text(title("Warp", "Slots"));

    private static final DialogElementLocale BODY = LangEntry.builder("Warps.Dialog.WarpSlots.Body").dialogElement(400,
        "Specify the " + YELLOW.wrap("slots") + " and " + YELLOW.wrap("page number") + " where this warp will be displayed in the GUI.",
        "",
        "Slot numbers start " + ORANGE.wrap("at 0") + " and should be separated by commas " + GRAY.wrap("(e.g., 1, 2, 10, 11)") + ".",
        "",
        "The " + YELLOW.wrap("total number") + " of GUI pages is determined by the " + YELLOW.wrap("highest") + " page number assigned to any warp.",
        "",
        "You can set the page number to " + RED.wrap("0") + " to hide this warp from the GUI."
    );

    private static final TextLocale INPUT_PAGE = LangEntry.builder("Warps.Dialog.WarpSlots.Input.Page").text(SPRITE_ITEM.apply(Material.PAPER) + " Page");
    private static final TextLocale INPUT_SLOTS = LangEntry.builder("Warps.Dialog.WarpSlots.Input.Name").text(SPRITE_ITEM.apply(Material.ITEM_FRAME) + " Slots");
    ...
    public WrappedDialog create(@NonNull Player player, @NonNull Warp warp) {
        return Dialogs.builder()
            .base(DialogBases.builder(TITLE)
                ...
                .inputs(
                    DialogInputs.text(JSON_PAGE, INPUT_PAGE).maxLength(2).initial(String.valueOf(warp.getMenuPage())).build(),
                    DialogInputs.text(JSON_SLOTS, INPUT_SLOTS).maxLength(30).initial(ArrayUtil.arrayToString(warp.getMenuSlots())).build()
                )
                .build()
            )
