...
public class KitCommandsDialog extends Dialog<Kit> {
    ...
    private static final TextLocale TITLE = LangEntry.builder("Kits.Dialog.KitCommands.Title").text(title("Kit",
        "Commands"));

    private static final DialogElementLocale BODY = LangEntry.builder("Kits.Dialog.KitCommands.Body").dialogElement(400,
        "Enter up to " + SOFT_YELLOW.wrap(COMMANDS_AMOUNT + " commands") +
            " that are executed when a player receives the kit. The commands will be executed in the same order as the fields listed below.",
        "",
        SOFT_YELLOW.wrap("→") + " Use the " + SOFT_YELLOW.wrap(PLAYER_NAME) +
            " placeholder to insert the name of the player who received the kit.",
        "",
        SOFT_YELLOW.wrap("→") + " You can use the internal " + SOFT_YELLOW.wrap("Kit") + " placeholders: click " +
            OPEN_URL.with(SLLinks.WIKI_PLACEHOLDERS).wrap(SOFT_GREEN.and(UNDERLINED).wrap("HERE")) +
            " to view documentation.",
        "",
        SOFT_YELLOW.wrap("→") + " You can use any placeholders from the " + SOFT_YELLOW.wrap(Plugins.PLACEHOLDER_API) +
            " plugin."
    );

    private static final TextLocale INPUT_COMMAND = LangEntry.builder("Kits.Dialog.KitCommands.Input.Command").text(
        "Command " + SOFT_YELLOW.wrap("#%s"));
    ...
    public WrappedDialog create(@NonNull Player player, @NonNull Kit kit) {
        ...
        for (int index = 0; index < size; index++) {
            inputs.add(DialogInputs.text(JSON_COMMAND.apply(index), INPUT_COMMAND.text().formatted(String.valueOf(
                index + 1)))
        ...
        return Dialogs.create(builder -> {
            ...
            builder.handleResponse(DialogActions.OK, (viewer, identifier, nbtHolder) -> {
                ...
                for (int index = 0; index < size; index++) {
                    nbtHolder.getText(JSON_COMMAND.apply(index)).filter(Predicate.not(String::isBlank)).ifPresent(
                        command -> {
