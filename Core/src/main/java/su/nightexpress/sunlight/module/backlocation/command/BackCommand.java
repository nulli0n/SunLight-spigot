...
public class BackCommand {
    ...
    public static final String NODE = "back";
    ...
    public static void load(@NotNull SunLightPlugin plugin, @NotNull BackLocationModule module) {
        ...
        CommandRegistry.addTemplate(NODE, CommandTemplate.direct(new String[]{NODE}, NODE));
    }
    ...
    public static DirectNodeBuilder builder(@NotNull SunLightPlugin plugin, @NotNull BackLocationModule module, @NotNull CommandTemplate template, @NotNull FileConfig config) {
        return DirectNode.builder(plugin, template.getAliases())
            ...
            .withArgument(ArgumentTypes.playerName(CommandArguments.PLAYER).permission(BackLocationPerms.COMMAND_BACK_OTHERS))
            .withFlag(CommandFlags.silent().permission(BackLocationPerms.COMMAND_BACK_OTHERS))
    ...
    public static boolean execute(@NotNull SunLightPlugin plugin, @NotNull BackLocationModule module, @NotNull CommandContext context, @NotNull ParsedArguments arguments) {
        ...
        if (!module.teleportToLocation(target, LocationType.PREVIOUS, silent)) {
            if (context.getSender() != target) {
                context.send(BackLocationLang.COMMAND_BACK_OTHERS_NOTHING.getMessage().replace(Placeholders.forPlayer(target)));
            }
            return false;
        }

        if (context.getSender() != target) {
            context.send(BackLocationLang.COMMAND_BACK_OTHERS_DONE.getMessage().replace(Placeholders.forPlayer(target)));
        }
        return true;
    }
}
