...
public class ChatSettings extends AbstractConfig {
    ...
    private final ConfigProperty<EventPriority> chatEventPriority = this.addProperty(ConfigTypes.forEnum(EventPriority.class), "Global.Chat-Event-Priority",
        EventPriority.HIGH,
    ...
    private final ConfigProperty<Boolean> discordHookEnabled = this.addProperty(ConfigTypes.BOOLEAN, "Global.DiscordSRV.Enabled",
        true,
        "Enables %s integration.".formatted(HookId.DISCORD_SRV),
        "[*] Experimental. Currently supports only Game -> Discord messages."
    );

    private final ConfigProperty<Boolean> disableReports = this.addProperty(ConfigTypes.BOOLEAN, "Global.Disable-Chat-Reports",
        true,
        "Disables the Chat Reports system and verification messages on join."
    );

    private final ConfigProperty<Boolean> roleplayEnabled = this.addProperty(ConfigTypes.BOOLEAN, "Roleplay-Commands.Enabled",
        true,
        "Adds a few roleplay-like commands if enabled."
    );

    private final ConfigProperty<String> roleplayMeFormat = this.addProperty(ConfigTypes.STRING, "Roleplay-Commands.Me.Format",
        ITALIC.wrap(SOFT_AQUA.wrap(PLAYER_DISPLAY_NAME) + " " + GRAY.wrap(SLPlaceholders.GENERIC_MESSAGE)),
        "Format for the /me command."
    );

    private final ConfigProperty<Map<String, FormatComponent>> formatComponents = this.addProperty(ConfigTypes.forMapWithLowerKeys(FORMAT_COMPONENT_CONFIG_TYPE),
    ...
    private final ConfigProperty<Map<String, FormatDefinition>> formatDefinitions = this.addProperty(ConfigTypes.forMapWithLowerKeys(FORMAT_DEFINITION_CONFIG_TYPE),
        ...
        "    - Ranks: List of ranks (permission group names) to which this format is applicable. Add '%s' to allow any rank.".formatted(SLPlaceholders.WILDCARD),
    ...
    private final ConfigProperty<Boolean> channelsEnabled = this.addProperty(ConfigTypes.BOOLEAN, "Channels.Enabled",
        ...
        "[*] When disabled, the chat fallbacks to the %s channel inside the %s directory and no other channel features are available to use.".formatted(ChatDefaults.DEFAULT_CHANNEL_ID, ChatFiles.DIR_CHANNELS)
    );

    private final ConfigProperty<String> channelsDefaultId = this.addProperty(ConfigTypes.STRING, "Channels.Default-Channel-Id",
    ...
    private final ConfigProperty<Boolean> conversationEnabled = this.addProperty(ConfigTypes.BOOLEAN, "Conversations.Enabled",
        true,
        "Enables the Conversations feature allowing players to send private messages to each other."
    );

    private final ConfigProperty<Boolean> conversationSoundEnabled = this.addProperty(ConfigTypes.BOOLEAN, "Conversations.Sound.Enabled",
        true,
        "If enabled, a sound will play whenever a player receives or sends a private message."
    );

    private final ConfigProperty<NightSound> conversationSoundIncoming = this.addProperty(ConfigTypes.NIGHT_SOUND, "Conversations.Sound.Incoming",
        VanillaSound.of(Sound.BLOCK_NOTE_BLOCK_BELL),
        "Sound for incoming private messages."
    );

    private final ConfigProperty<NightSound> conversationSoundOutgoing = this.addProperty(ConfigTypes.NIGHT_SOUND, "Conversations.Sound.Outgoing",
        VanillaSound.of(Sound.UI_BUTTON_CLICK),
        "Sound for outgoing private messages."
    );

    private final ConfigProperty<String> conversationProxyFormat = this.addProperty(ConfigTypes.STRING, "Conversations.Format.Basic",
    ...
    private final ConfigProperty<String> conversationIncomingFormat = this.addProperty(ConfigTypes.STRING, "Conversations.Format.Incoming",
        LIGHT_PURPLE.wrap("[Whisper]") + " " + SOFT_PINK.wrap(PLAYER_DISPLAY_NAME + ": ") + GRAY.wrap(SLPlaceholders.GENERIC_FORMAT) + " " +
        ...
        "[>] Use the %s placeholder to insert 'basic' conversation format from the 'Format.Basic' section above".formatted(SLPlaceholders.GENERIC_FORMAT),
        "[*] For a list of placeholders, please see the 'Format.Basic' section above."
    );

    private final ConfigProperty<String> conversationOutgoingFormat = this.addProperty(ConfigTypes.STRING, "Conversations.Format.Outgoing",
        LIGHT_PURPLE.wrap("[Whisper]") + " " + SOFT_PINK.wrap("You ➡ " + PLAYER_DISPLAY_NAME + ": ") + GRAY.wrap(SLPlaceholders.GENERIC_FORMAT),
    ...
    private final ConfigProperty<Map<String, GroupMention>> mentionsCustoms = this.addProperty(ConfigTypes.forMap(GROUP_MENTION_CONFIG_TYPE),
        ...
        "    - Ranks: List of ranks (permission group names) to which this mention is applicable. Add '%s' to allow any rank.".formatted(SLPlaceholders.WILDCARD)
    );

    private final ConfigProperty<Integer> autoModerationUserCacheLifetime = this.addProperty(ConfigTypes.INT, "AutoModeration.UserCache.Life-Time",
        60,
    ...
    private final ConfigProperty<Boolean> antiCapsEnabled = this.addProperty(ConfigTypes.BOOLEAN, "AutoModeration.Anti-Caps.Enabled",
        true,
    ...
    private final ConfigProperty<Integer> antiCapsLengthThreshold = this.addProperty(ConfigTypes.INT, "AutoModeration.Anti-Caps.Min-Message-Length",
        5,
        "The minimum message length required for the Anti-Caps feature to trigger."
    );

    private final ConfigProperty<Integer> antiCapsUpperCaseThreshold = this.addProperty(ConfigTypes.INT, "AutoModeration.Anti-Caps.Uppercase-Threshold",
        70,
        "The minimum threshold (in %) of uppercase characters in a message required for the Anti-Caps feature to trigger."
    );

    private final ConfigProperty<Set<String>> antiCapsAffectedCommands = this.addProperty(ConfigTypes.STRING_SET_LOWER_CASE, "AutoModeration.Anti-Caps.Affected-Commands",
        Set.of("tell", "me", "reply", "broadcast"),
        "List of commands for which the Anti-Caps feature will be active."
    );

    private final ConfigProperty<Boolean> antiFloodEnabled = this.addProperty(ConfigTypes.BOOLEAN, "AutoModeration.Anti-Flood.Enabled",
        true,
    ...
    private final ConfigProperty<Double> antiFloodSimilarityScoreThreshold = this.addProperty(ConfigTypes.DOUBLE, "AutoModeration.Anti-Flood.Similarity.Score-Threshold",
        80D,
    ...
    private final ConfigProperty<Integer> antiFloodSimilarityCountThreshold = this.addProperty(ConfigTypes.INT, "AutoModeration.Anti-Flood.Similarity.Count-Threshold",
        3,
    ...
    private final ConfigProperty<Long> antiFloodCommandCooldown = this.addProperty(ConfigTypes.LONG, "AutoModeration.Anti-Flood.Commands.Cooldown",
        500L,
        "Sets global cooldown (in milliseconds) on command usage..",
        "[Default is 500ms (0.5s)]"
    );

    private final ConfigProperty<Set<String>> antiFloodCommandWhitelist = this.addProperty(ConfigTypes.STRING_SET_LOWER_CASE, "AutoModeration.Anti-Flood.Command-Whitelist",
    ...
    private final ConfigProperty<Boolean> profanityFilterEnabled = this.addProperty(ConfigTypes.BOOLEAN, "AutoModeration.Profanity-Filter.Enabled",
        true,
        "This feature enables a filter for profanity (or any specified words) in messages and commands."
    );

    private final ConfigProperty<Set<String>> profanityFilterRules = this.addProperty(ConfigTypes.STRING_SET_LOWER_CASE, "AutoModeration.Profanity-Filter.Rules",
    ...
    private final ConfigProperty<Set<String>> profanityFilterAffectedCommands = this.addProperty(ConfigTypes.STRING_SET_LOWER_CASE, "AutoModeration.Profanity-Filter.Affected-Commands",
        Set.of("tell", "me", "reply", "broadcast"),
    ...
    private final ConfigProperty<String> itemShowPlaceholder = this.addProperty(ConfigTypes.STRING, "ItemShow.Placeholder",
        "@hand",
    ...
    private final ConfigProperty<Map<SpyType, String>> spyFormat = this.addProperty(ConfigTypes.forMap(string -> Enums.get(string, SpyType.class), Enum::name, ConfigTypes.STRING),
        "Spy.Info-Format",
