...
public class SpawnRulesDialog extends Dialog<Spawn> {
    ...
    private static final TextLocale TITLE = LangEntry.builder("Spawns.Dialog.SpawnLoginRules.Title").text(title("Spawn", "Login Rules"));

    private static final DialogElementLocale BODY = LangEntry.builder("Spawns.Dialog.SpawnLoginRules.Body").dialogElement(400,
        " "
    );

    private static final TextLocale INPUT_ENABLED = LangEntry.builder("Spawns.Dialog.SpawnLoginRules.Input.Enabled").text("Enabled");
    private static final TextLocale INPUT_RANKS = LangEntry.builder("Spawns.Dialog.SpawnLoginRules.Input.Ranks").text("Affected Ranks");
    ...
    public WrappedDialog create(@NotNull Player player, @NotNull Spawn spawn) {
        return Dialogs.builder()
            .base(DialogBases.builder(TITLE)
                ...
                .inputs(
                    DialogInputs.bool(JSON_ENABLED, INPUT_ENABLED).initial(this.ruleFunction.apply(spawn).isEnabled()).build(),
