...
public class EnderchestCopyCommand extends AbstractCommand<SunLight> {

    public static final String NAME = "copy";

    public EnderchestCopyCommand(@NotNull SunLight plugin) {
        super(plugin, new String[]{NAME}, Perms.COMMAND_ENDERCHEST_COPY);
        this.setDescription(plugin.getMessage(Lang.COMMAND_ENDERCHEST_COPY_DESC));
        this.setUsage(plugin.getMessage(Lang.COMMAND_ENDERCHEST_COPY_USAGE));
    }

    @Override
    @NotNull
    public List<String> getTab(@NotNull Player player, int arg, @NotNull String[] args) {
        if (arg == 1 || (arg == 2 && player.hasPermission(Perms.COMMAND_ENDERCHEST_COPY_OTHERS))) {
    ...
    protected void onExecute(@NotNull CommandSender sender, @NotNull CommandResult result) {
        ...
        if (result.length() >= 3 && !sender.hasPermission(Perms.COMMAND_ENDERCHEST_COPY_OTHERS)) {
        ...
        if (sender == source && sender == target) {
        ...
        if (sender == target) {
            ...
            plugin.getMessage(Lang.COMMAND_ENDERCHEST_COPY_DONE_OTHERS)
